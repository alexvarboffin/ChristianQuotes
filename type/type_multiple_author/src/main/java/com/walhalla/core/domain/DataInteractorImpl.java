...
public class DataInteractorImpl extends AbstractInteractor implements LocalDataBaseInteractor<Status> {
    ...
    public DataInteractorImpl(Executor threadExecutor, MainThread mainThread, StatusDao statusDao, CategoryDao categoryDao/*, AuthorDao authorDao*/) {
...
//        });
    ...
    @Override
    public void getByAuthorName(String authorName, Callback<List<Status>> callback) {
        String like = "%" + authorName + "%";
        mThreadExecutor.execute(() -> {
            try {
                final List<Status> list = statusDao.getByAuthorLike(like);
                if (callback != null) {
                    if (!list.isEmpty()) {
                        mMainThread.post(() -> callback.onMessageRetrieved(list));
                    } else {
    ...
    public void getCategoryNamesByIds(List<Integer> cIdList, Callback<List<String>> callback) {
        mThreadExecutor.execute(() -> {
            try {
                List<String> list = categoryDao.getCategoryNamesByIds(cIdList);
                if (callback != null) {
                    if (!list.isEmpty()) {
                        mMainThread.post(() -> callback.onMessageRetrieved(list));
                    } else {
