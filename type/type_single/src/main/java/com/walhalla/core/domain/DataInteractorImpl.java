...
public class DataInteractorImpl extends AbstractInteractor
        implements LocalDataBaseInteractor<Status> {
...
//        });
    ...
    public void getFullData(long _id, Callback<List<Status>> callback) {
        mThreadExecutor.execute(() -> {
            try {
                ...
                final List<Status> list = dao.getFullData(_id);
                if (callback != null) {
                    if (!list.isEmpty()) {
                        mMainThread.post(() -> callback.onMessageRetrieved(list));
                    } else {
    ...
    @Override
    public void getLikeData(String _id, Callback<List<Status>> callback) {
        mThreadExecutor.execute(() -> {
            try {

                final List<Status> list = dao.getLike(_id);
                if (callback != null) {
                    if (list.isEmpty()) {
                        mMainThread.post(() -> callback.onRetrievalFailed("Database is empty, reinstall the application"));
                    } else {
    ...
    public void getFavorite(Callback<List<Status>> callback) {
        mThreadExecutor.execute(() -> {
            try {
                final List<Status> list = dao.getFavorite();
                if (callback != null) {
                    mMainThread.post(() -> callback.onMessageRetrieved(list));
                }
            } catch (Exception e) {
    ...
    public void randomStatus(int _id, Callback<Status> callback) {
        mThreadExecutor.execute(() -> {
            try {
                final Status status = dao.getById(_id);
                if (callback != null) {
                    if (status == null) {
                        mMainThread.post(() -> callback.onRetrievalFailed("<" + _id + ">"));
                    } else {
