...
import com.hw.photomovie.sample.UriUtil;
import com.hw.photomovie.sample.utils.IntentUtils;
...
import com.hw.photomovie.samples.R;
...
public class DemoPresenter implements MovieFilterView.FilterCallback, IMovieTimer.MovieListener, MovieTransferView.TransferCallback {
    ...
    public DemoPresenter(Activity activity, IDemoView demoView) {
        mDemoView = demoView;
    ...
    private void initMoviePlayer() {
        ...
        addWaterMark();
        mPhotoMoviePlayer = new PhotoMoviePlayer(activity.getApplicationContext());
...
//        mMovieRenderer.setWaterMark("Watermark",40, Color.argb(100,255,0,0),100,100);
    ...
    public void onTransferSelect(TransferItem item) {
        ...
        mPhotoMoviePlayer.setDataSource(mPhotoMovie);
        if (mMusicUri != null) {
            mPhotoMoviePlayer.setMusic(activity, mMusicUri);
        }
        mPhotoMoviePlayer.setOnPreparedListener(new PhotoMoviePlayer.OnPreparedListener() {
            @Override
            public void onPreparing(PhotoMoviePlayer moviePlayer, float progress) {
    ...
    public void setMusic(Uri uri) {
        mMusicUri = uri;
        mPhotoMoviePlayer.setMusic(activity, uri);
    }
        ...
        recorder.startRecord(new GLMovieRecorder.OnRecordListener() {
            ...
            public void onRecordFinish(boolean success) {
                ...
                if (success) {
                    ...
                    IntentUtils.shareVideo(mDemoView.getActivity(), file);
                } else {
    ...
    public void onPhotoPick(List<Uri> photos) {
        List<PhotoData> photoDataList = new ArrayList<PhotoData>(photos.size());
        for (Uri path : photos) {
            PhotoData photoData = new SimplePhotoData(activity, path.getPath(), PhotoData.STATE_LOCAL);
    ...
    public void onPhotoPick0(List<String> photos) {
        List<PhotoData> photoDataList = new ArrayList<PhotoData>(photos.size());
